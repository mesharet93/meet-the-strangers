...
const createPeerConnection = () => {
  ...
  // receiving tracks
  const remoteStreams = new MediaStream();
  store.setRemoteStream(remoteStreams);
  ui.updateRemoteVideo(remoteStreams);
...
export const handlePreOfferAnswer = (data) => {
  ...
  if (preOfferAnswer === constants.preOfferAnswer.CALL_REJECTED) {
    setIncomingCallsAvailable();
    ui.showInfoDialog(preOfferAnswer);
    // show dialog that the call is rejected by the callee
  }
...
export const handleWebRTCCandidate = async (data) => {
  try {
    ...
    console.error(
      "error occured when trying to add recieved ice candidta",
      err
    );
  }
};
...
export const swtichBetweenCameraAndScreenSharing = async (
  screenSharingActive
) => {
